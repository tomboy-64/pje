...
processMatrix matrix = pM initial
	where
		...
		myOrd ((_,x),a) ((_,y),b) = if a > b then GT
					else if a < b then LT
					else if x > y then GT
					else if x < y then LT
					else EQ
		...
		pM :: [((Int,Int),Int)] -> ((Int,Int),Int)
		pM (input:rest) =
			if snd (fst input) == (length (head matrix)) -1
			then input
			else
...
--						 (\list into -> insertBy myOrd into list) rest (nextCoords input)
						...
						newList b a = case lookup (fst a) b of
							Nothing -> insertBy myOrd a b
							Just x -> if x <= snd a
								then b
								else insertBy myOrd a (deleteBy (\(x,_) (y,_) -> x==y) a b)
						unjinx = (head next):(filter (\x -> fst x /= fst (head next)) (tail next))


		
		nextCoords ((a,b),z) = up ++ down ++ right
			where
				up = if a > 0
				...
				right = if b < (length (matrix !! 0)) -1
					...
					else error "Hmmmmmm ... shouldn't we be done already?"
